...
class TotalController extends Controller
{
    ...
    public function tampilPertanggal($tgl)
    {
        ...
        $total = $tabung->sum('uang') + $tot->sum('saldo') - $tat->sum('nominal');
        $transaksi = new Tabungan;
        $transaksi->uang = $total;
        $transaksi->save();

        $sisa =  Tabungan::where('id')->update(['uang' => $hasil_perhitungan, 'tanggal' => $tgl]);
        return view('total.totalcetak', compact('tot', 'tat', 'tabung', 'tanggal', 'sisa', "hasil_perhitungan"));
    }
}
