            ...
            <div class="table-responsive p-0">
                ...
                <table class="table table-bordered">
                    ...
                    @foreach ($tabungan as $item)
                        ...
                        <td class="align-middle text-center text-sm">{{ $item->tabungan }}</td>
                        <td>
                            <form action="{{ route('saldos.destroy',$pengeluaran->id) }}" method="POST">
                            <div class="align-middle text-center text-sm">
                                <a class="btn btn-primary" href="{{ route('saldos.edit',$item->id) }}">Edit</a>
                                @csrf
                                @method('DELETE')
                            </div>
                            </form>
